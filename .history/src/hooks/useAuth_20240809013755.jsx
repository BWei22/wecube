...
function useProvideAuth() {
  ...
  const signinWithGoogle = async () => {
    ...
    if (userDocSnap.exists()) {
      ...
      logState("User signed in with Google");
      ...
      logState("New Google user created, redirected to complete profile");
      navigate('/complete-profile');
    }
  };
  ...
  const signupWithEmailAndPassword = async (email, password) => {
    ...
    setUser(user);
    setProfileComplete(false);
    logState("User signed up with email, redirected to complete profile");
    navigate('/complete-profile');
  };
  ...
  const signinWithEmailAndPassword = async (email, password) => {
    ...
    if (userDocSnap.exists()) {
      ...
      setUsername(userData.username);
      setProfileComplete(userData.profileComplete);
      logState("User signed in with email and password");
      if (userData.profileComplete) {
        navigate('/competitions');
      } else {
        navigate('/complete-profile');
      ...
      console.log("No user document found, redirecting to signup");
      navigate('/signup');
    }
  };
    ...
    const unsubscribe = auth.onAuthStateChanged(async (user) => {
      if (user) {
        ...
        if (userDocSnap.exists()) {
          ...
          setUser(user);
          setProfileComplete(false);
          logState("Auth state changed: new user, redirecting to complete profile");
          navigate('/complete-profile');
        }
      } else {
